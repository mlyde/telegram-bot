...
def logSendMessageContent(message: Message, type: str, is_edit: bool, info: str = None):
    """整合发送消息日志"""
    ...
    # 媒体消息
    log_text = (
        (f"send edit {type}({message.id})" if is_edit else f"send {type}({message.id})")
        + (f" reply to message({message.reply_to_message.id})" if is_reply else '')
        + (f" with \"{caption}\" caption" if caption else '')
        + f" from {getUserInfo(message.from_user)} in {getChatInfo(message.chat)} at {message.date}"
        + (f": {info}" if info else '')
    )
...
async def sendPhoto(update: Update, context: ContextTypes.DEFAULT_TYPE, img=None):
    ...
    if img is None:
        ...
        # 发送一张网络图片
        message_sent = await message.reply_photo(photo="https://www.baidu.com/favicon.ico")
        ...
        message_sent = await message.reply_photo(photo=img)

    logSendMessageContent(message_sent, "photo", False, getPhotoSizeInfo(message_sent.photo))
...
async def sendAudio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    message_sent = await message.reply_audio(audio=open(f"{directory}/考试什么的都去死吧.mp3", 'rb'))
    logSendMessageContent(message_sent, "audio", False, getAudioInfo(message.audio))
...
async def sendEasterEggReply(message: Message) -> bool:
    ...
    if today.month == 4 and today.day == 1:
        message_sent = await message.reply_text("418 I'm a teapot", reply_markup=ReplyKeyboardRemove())
...
async def sendRandomStartReply(message: Message):
    ...
    match choice:
        case 'g':
            message_sent = await message.reply_text("Let's get started.", reply_markup=ReplyKeyboardRemove())
        case 'h':
            message_sent = await message.reply_sticker(sticker='CAACAgUAAxkBAAICBGaThaP_3NPC0J301sJxAkwv81wZAAKNCQACCYSJVmS11JMf6Da9NQQ')   # 贴纸内容: 柴郡头猫猫趴在西瓜上
            is_log = True
            logSendMessageContent(message_sent, "sticker", False, 'sticker_id: CAACAgUAAxkBAAICBGaThaP_3NPC0J301sJxAkwv81wZAAKNCQACCYSJVmS11JMf6Da9NQQ')
        case 'i':
            message_sent = await message.reply_text("emm, may I help you?", reply_markup=ReplyKeyboardRemove())
        case _:
            message_sent = await message.reply_text("Hello! I am a bot. <b>^_^</b>", reply_markup=ReplyKeyboardRemove(), parse_mode=ParseMode.HTML)
...
async def sendCaptchaMessage(context: ContextTypes.DEFAULT_TYPE, chat: Chat, user: User):
    ...
    context.job_queue.run_once(
        ...
        data={"chat": chat, "user": user, "message_id": message_sent.message_id},
    )
