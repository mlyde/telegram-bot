...
async def getStickerSetInfo(context: ContextTypes.DEFAULT_TYPE, set_name: str) -> str:
    """sticker set 贴纸包信息"""
...
def getCommonFileInfo(file: Document | Any) -> str:
    """document 一般文件的通用信息"""
    name = file.file_name
    ...
    return ((f"({name})" if isinstance(file, Document) else '')
        + f"({unique_id})({mime_type})(size: {size})")
...
def getPhotoSizeInfo(photo_size: tuple[PhotoSize] | PhotoSize) -> str:
    """照片信息"""
    if isinstance(photo_size, tuple):
        # [-1] 为原图
